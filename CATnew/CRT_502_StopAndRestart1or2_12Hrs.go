	...
	// CRT_502_StopAndRestart1or2_12Hrs.go
        ...
        for cycle < numCycles {
                ...
                for j < chco2.NumberOfPeersInNetwork {
                        ...
                        totalStopCntr++
                        if totalStopCntr % 5 == 0 {
                                // take down a second node every 5th trip. Alternately, select preceding node every 3rd time.
                                doubleDown := j + 1
                                if totalStopCntr % 3 == 0 { doubleDown = j - 1 }

                                // wraparound cases
                                if doubleDown < 0 { doubleDown = chco2.NumberOfPeersInNetwork - 1 }
                                if doubleDown >= chco2.NumberOfPeersInNetwork { doubleDown = 0 }

                                // SDK limitation requires listing the lower one first
                                if doubleDown > j { chco2.StopPeers([]int{ j, doubleDown })
				...
				chco2.InvokeOnEachPeer(chco2.DefaultInvokesPerPeer)
                                ...
                                } else {            chco2.RestartPeers([]int{ doubleDown, j }) }
